...
export function MaintenancePlanForm({
 ...
 <TouchableOpacity
 key={vehicle.id}
 className={`rounded-lg border px-3 py-2 ${selected ? 'border-brand-500 bg-primary-container/10 dark:border-dark-brand-500 ' : 'border-outline-variant '}`}
 onPress={() => setValue('vehicleId', vehicle.id, { shouldValidate: true })}
 >
 <Text
 className={`font-sans text-sm ${selected ? 'font-bold text-primary ' : 'text-on-surface-variant '}`}
 >
