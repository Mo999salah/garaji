...
export function RequestTimeline({ currentStatus, events }: RequestTimelineProps) {
  if (currentStatus === 'cancelled') {
    ...
    return (
      <View className="bg-surface-container-lowest rounded-[20px] shadow-[0px_4px_20px_rgba(0,0,0,0.04)] p-6 relative">
        <View className="flex-row-reverse items-start gap-4">
          ...
          <View className="flex-col items-end">
            ...
            {cancelEvent?.note ? (
              <Text className="font-label-sm text-[13px] leading-[18px] text-on-surface-variant mt-1 text-right">{cancelEvent.note}</Text>
            ) : null}
            {cancelEvent ? (
              <Text className="font-label-sm text-[13px] leading-[18px] text-on-surface-variant mt-1">{formatEventDate(cancelEvent.createdAt)}</Text>
            ) : null}
          </View>
        </View>
      </View>
          ...
          return (
            ...
            >
              ...
              <View className="flex-col items-end">
                ...
                <Text className="font-label-sm text-[13px] leading-[18px] text-on-surface-variant mt-1 text-right">
