...
interface RequestCardProps {
  ...
  vehicleName?: string;
  serviceName?: string;
  /** Resolved branch name — pass from parent via branchId lookup */
  branchName?: string;
}
...
function getBorderColor(status: ServiceRequestStatus) {
  switch (status) {
    case 'pending': return 'border-warning/30';
...
export function RequestCard({
  ...
  // In RTL, the "forward" indicator points left
  const chevronName = I18nManager.isRTL ? "chevron-left" : "chevron-right";

  const locationLabel =
    request.requestType === 'branch_appointment'
      ? (branchName ?? 'فرع غير محدد')
      : 'خدمة بالموقع';

  return (
    <AnimatedPressable
      accessibilityLabel={`طلب ${REQUEST_TYPE_LABELS[request.requestType]} - ${vehicleName ?? 'مركبة'}`}
      accessibilityHint="اضغط لعرض تفاصيل الطلب"
    ...
    >
      ...
      {/* Bottom metadata — use flex-row (natural RTL) instead of flex-row-reverse to avoid double-flip */}
      <View className="flex-row justify-between items-center mt-2 pt-4 border-t border-surface-container-high/50">
        <MaterialIcons name={chevronName} size={24} color={AppColors.outlineVariant} />
        <View className="flex-row items-center gap-4">
          ...
          <View className="flex-row items-center gap-1.5">
            <Text className="font-label-sm text-label-sm text-on-surface-variant">{locationLabel}</Text>
            <MaterialIcons name={request.requestType === 'branch_appointment' ? 'location-on' : 'my-location'} size={18} color={AppColors.primary} />
          </View>
        </View>
      </View>
    </AnimatedPressable>
