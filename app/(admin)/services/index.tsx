...
function AdminServiceCard({ service, onPress }: { service: Service; onPress: () => void }) {
 ...
 <Pressable 
 onPress={onPress}
 className={`bg-surface rounded-2xl shadow-soft p-4 flex-col gap-1 relative mb-4 active:scale-[0.98] ${!service.isActive ? 'opacity-75' : ''}`}
 >
 ...
 {service.description ? (
 <Text className="font-label-sm text-label-sm text-on-surface-variant text-right">{service.description}</Text>
 ) : null}
 </View>
 ...
 <Text className="font-label-sm text-label-sm">{service.durationMinutes} دقيقة</Text>
 </View>
 )}
 </View>
 </Pressable>
...
export default function AdminServicesScreen() {
 ...
 {/* Top Navigation Bar */}
 <View className="flex-row-reverse justify-between items-center px-margin-mobile h-16 w-full z-40 bg-surface shadow-soft">
 <Pressable 
 onPress={() => router.back()} 
